...
const sentryDsn = process.env.MIRZA_SENTRY_DSN_PRODUCTION ?? '';
const sentryDsnLiteral = sentryDsn ? JSON.stringify(sentryDsn) : "''";

const source = readFileSync(sourcePath, 'utf8');
const generated = source.replace(/sentryDsn:\s*(['"`])[\s\S]*?\1,/, () => `sentryDsn: ${sentryDsnLiteral},`);

if (generated === source && !source.includes('sentryDsn')) {
  throw new Error('Could not find sentryDsn in Mirza production environment.');
}

writeFileSync(sourcePath, generated);
console.log(`Generated Mirza production environment with Sentry DSN ${sentryDsn ? 'configured' : 'empty'}.`);
