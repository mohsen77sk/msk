...
import { TopProductReport } from '../../dashboard.types';

@Component({
  selector: 'mz-top-products',
...
export class DashboardTopProductsComponent implements OnInit {
  ...
  private _items: TopProductReport[] = [];
    ...
    this.dateRange.valueChanges
      .pipe(
        ...
        switchMap((value) => {
          ...
          return this._dashboardService.getTopProducts(
   ...
   */
  generateChartData(value: TopProductReport[]): void {
    this._items = value;
    this.chartOptions = {
      chart: {
        animations: {
      ...
      xaxis: {
        categories: value.map((x) => x.productName),
        labels: {
          style: {
